...
import { imageCategorySeoArticle } from "@/lib/content/categoryPagesContent";
import { imageToolsGrid, imageToolsPageContent } from "@/lib/content/homepageData";
import { seoData, toMetadata } from "@/lib/content/seoData";

export const metadata: Metadata = toMetadata(seoData.imageTools);

export default function ImageToolsPage() {
  const c = imageToolsPageContent;
    ...
    <section className="bg-surface pt-4 pb-12 md:pt-8 md:pb-16 lg:pt-10 lg:pb-20">
      <Container>
        ...
        >
          ...
          <span className="text-secondary-text/50">/</span>
          <span className="text-secondary-text">Image tools</span>
        </nav>
        ...
        <div className="mt-10 flex flex-wrap gap-3 text-sm font-semibold">
          <Link href="/developer-tools" className="rounded-lg border border-input-border bg-background px-4 py-2 no-underline hover:border-primary/40 hover:text-primary">
            Developer tools
          </Link>
          <Link href="/text-tools" className="rounded-lg border border-input-border bg-background px-4 py-2 no-underline hover:border-primary/40 hover:text-primary">
            Text tools
          </Link>
          <Link href="/tools" className="rounded-lg border border-input-border bg-background px-4 py-2 no-underline hover:border-primary/40 hover:text-primary">
            All tools directory
          </Link>
        </div>
